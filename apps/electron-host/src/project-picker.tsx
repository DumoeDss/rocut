...
/**
 * The Vite example's project picker, mirrored — host code, not editor code.
 * The Next app has its own `/projects` page and this stands in for it on the
 * desktop host. It exists so "create or open a project" is reachable without
 * hand-crafting a project id, which means the identity seam gets exercised
 * honestly rather than bypassed.
 *
 * Persistence is acquired from the session-owned editor coordinator, whose
 * store is whatever the Electron host composed (the in-memory reference store
 * at the skeleton stage, `FilesystemProjectStore` from Group 4 on). Creating
 * and opening still go through `EditorCore`, because those load a project
 * *into the editor* rather than merely reading storage.
 */
export function ProjectPicker({
	...
	const refresh = useCallback(async () => {
		try {
			...
			// Capacity is null when the store declines to estimate. Say so
			// rather than rendering a confident 0 MB.
			setStorageUsage(
				info.availability !== "available" || info.capacity === null
					? "unknown (store gave no estimate)"
