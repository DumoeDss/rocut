...
import { deriveGraphicsReport, UNIMPLEMENTED_RUNTIME_GRAPHICS } from "@/editor/ports";
...
/**
 * Stores whose migration has already been run.
 *
 ...
 *
 * A `WeakSet` rather than a flag on the session, so "once" means once per store
 * across every session that shares it, and so a store that is discarded does not
 * keep an entry alive.
 */
...
export async function createEditorSession(
	...
	function onRootUnmounted(): void {
		root = null;
		if (state === "mounted") state = "created";
		notify();
	}
...
async function runMigrationOnce(args: {
	...
	if (!store.migrate || migratedStores.has(store)) return;
	migratedStores.add(store);

	const to = store.schemaVersion;
	diagnostics.event({ event: { kind: "migration-started", from: to, to } });
	...
	if (outcome.status === "failed") {
		diagnostics.event({
			event: { kind: "migration-failed", reason: outcome.reason },
		});
		return;
	}
	diagnostics.event({
