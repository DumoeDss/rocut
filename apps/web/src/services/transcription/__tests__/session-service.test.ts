...
class FakeHandle implements WorkerHandle {
	...
	readonly sent: Array<{ message: unknown; transfer?: readonly Transferable[] }> = [];
	readonly messages = new Set<(event: WorkerMessageEvent) => void>();
	readonly errors = new Set<(event: WorkerErrorEvent) => void>();
	terminations = 0;
	postMessage(args: { message: unknown; transfer?: readonly Transferable[] }): void {
		this.sent.push(args);
	}
...
describe("session-owned transcription service", () => {
	test("requests a named module Worker through SessionResources and preserves the message flow", async () => {
		...
		expect(requests[0]?.url.toString()).toBe("https://editor.invalid/transcription.js");
		handle.emit({ type: "init-progress", progress: 25 });
		handle.emit({ type: "init-complete" });
		await afterTurn();
		expect(handle.sent[1]?.message).toMatchObject({ type: "transcribe", audio });
		...
		expect(await result).toEqual({ text: "hello", segments: [], language: "auto" });
	...
	test("keeps two sessions isolated and terminates each registry handle exactly once", async () => {
		const one = fixture();
		const two = fixture();
		const first = createTranscriptionService({ resources: one.resources, workerUrl: new URL("https://one.invalid/w.js") });
		const second = createTranscriptionService({ resources: two.resources, workerUrl: new URL("https://two.invalid/w.js") });
