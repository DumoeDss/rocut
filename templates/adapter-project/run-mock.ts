...
async function main(): Promise<number> {
	line(
		"custom-storage example: mock-installed leg (the real published chain, through the wasm test mock)",
	);
	line(
	...
	line(
		"wasm mock: installed (experimental entry @opencut/editor-classic/evidence/wasm-test-mock)",
	);
	...
	if (!chain) {
		line(
			"mock leg: FAILED — the classic chain still did not load with the wasm mock installed",
		);
		return 1;
	}
	line(
		`classic chain: loaded through the mock (${chain.steps.length} steps, target v${chain.target})`,
	);
	if (chain.steps.length === 0 || chain.target <= 1) {
		line(
			"mock leg: FAILED — the published chain is empty or trivial; nothing to validate against",
		);
		return 1;
	}
	...
	try {
		...
		line(
			`mock leg: FAILED — ${error instanceof Error ? error.message : String(error)}`,
		);
		return 1;
	}
	...
	const report = await runPortConformance({
		...
		label:
			"custom-storage example ports (migration exercised, mock-installed chain)",
		storeFixture: {
			store,
	...
	if (!report.passed) {
		line(
			`mock leg: FAILED — ports suite with migration exercised did not pass (cases=${report.results.length})`,
		);
		return 1;
	}
