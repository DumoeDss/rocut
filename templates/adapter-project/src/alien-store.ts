...
function identitiesConflict(
	...
	// Library namespaces conflict with library identities in the same
	// namespace and with nothing else. (Straight-line branches, each property
	// locally guarded — correlation-heavy formulations defeat strict
	...
	if (left.kind === "library-namespace") {
		return right.kind === "library-record" && right.namespace === left.namespace;
	}
	if (right.kind === "library-namespace") {
		return left.kind === "library-record" && left.namespace === right.namespace;
	}
