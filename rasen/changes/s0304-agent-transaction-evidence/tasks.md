...
- [x] 8.7 Regenerate `SOURCE_INVENTORY.{md,json}` with the committed generator rather than by hand — run **after** the implementation commit, because the generator inventories tracked files only; the regeneration also absorbed drift left by earlier changes that never regenerated it, attributed by count in `evidence/ship-log.md`
